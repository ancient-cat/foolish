...
export class Pinch extends Plugin {
  ...
  public move(e: FederatedPointerEvent): boolean {
    ...
    if (pointers.length >= 2) {
      ...
      const last =
        first.last && second.last ? Math.sqrt(Math.pow(second.last.x - first.last.x, 2) + Math.pow(second.last.y - first.last.y, 2)) : null;
      ...
      if (last) {
        ...
        let dist = Math.sqrt(
          Math.pow((second.last as PointData).x - (first.last as PointData).x, 2) +
